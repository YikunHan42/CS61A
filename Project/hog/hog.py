...
def roll_dice(num_rolls, dice=six_sided):
    ...
    while(i <= num_rolls):
        currentnum = dice()
        """temporary variable is a must, 
        since using dice() repeatedly actually means throwing a dice"""
        if(currentnum) == 1: pigoutflag = 1
        sum += currentnum
        i += 1
    return sum if pigoutflag == 0 else 1
    # END PROBLEM 1
...
def is_swap(player_score, opponent_score):
    ...
    "*** YOUR CODE HERE ***"
    # END PROBLEM 4
    playeronesdigit = player_score % 10
    opponenttensdigit = opponent_score % 100 // 10
    opponentonesdigit = opponent_score % 10
    if abs(playeronesdigit - opponentonesdigit) == opponenttensdigit: return True
    else: return False
