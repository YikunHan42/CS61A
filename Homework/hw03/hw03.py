...
def composer(func=lambda x: x):
    ...
    # inside-out, not like f(g(x)) which is composing inversely
...
def g_iter(n):
    ...
    if n == 1: return 1
        ...
        i = 0
        while(i < n - 3):
            new = 3 * backwards3 + 2 * backwards2 + backwards1
            backwards3 = backwards2
...
def missing_digits(n):
    ...
    if all_but_last == 0:
        ...
        if last - second_last <= 1:
            return missing_digits(all_but_last) + count
        else:
            count += last - second_last - 1
            return missing_digits(all_but_last) + count
    # sorted!
...
def count_change(total):
    ...
    "*** YOUR CODE HERE ***"
    def power2(total):
        if total == 0 or total == 1: return 0
        else:
            return 1 + power2(total // 2)

    Max = pow(2, power2(total))
    def count_partitions(total, Max):
        if total == 0:
            return 1
        if total < 0:
            return 0
        if Max == 0:
            ...
            with_Max = count_partitions(total - Max, Max)
            without_Max = count_partitions(total, Max // 2)
            return with_Max + without_Max
    return(count_partitions(total, Max))
